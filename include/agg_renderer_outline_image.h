...
namespace agg
{
    ...
    public:
		...
		template<class Source> void create(const Source& src, long thick)
		{
			m_height = uceil(src.height() * thick);
			m_width = uceil(src.width());
			m_width_hr = uround(src.width() * line_subpixel_scale);
			m_half_height_hr = uround(src.height() * thick * line_subpixel_scale / 2);
			m_offset_y_hr = m_dilation_hr + m_half_height_hr - line_subpixel_scale / 2;
			m_half_height_hr += line_subpixel_scale / 2;
        ...
        {
			color_type **bufff = (color_type **)m_buf.rows();
            m_filter->pixel_high_res(bufff,
                                     p, 
                                     x + m_dilation_hr,
                                     y + m_offset_y_hr, m_width_hr);
        }
    ...
    class AGG_API distance_interpolator4
    {
    public:
        //---------------------------------------------------------------------
        distance_interpolator4() {}
