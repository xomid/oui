...
namespace agg
{
    ...
    //=======================================================================
    struct AGG_API order_rgb  { enum rgb_e  { R=0, G=1, B=2, N=3 }; };
    struct AGG_API order_bgr  { enum bgr_e  { B=0, G=1, R=2, N=3 }; };
    struct AGG_API order_rgba { enum rgba_e { R=0, G=1, B=2, A=3, N=4 }; };
    struct AGG_API order_argb { enum argb_e { A=0, R=1, G=2, B=3, N=4 }; };
    struct AGG_API order_abgr { enum abgr_e { A=0, B=1, G=2, R=3, N=4 }; };
    struct AGG_API order_bgra { enum bgra_e { B=0, G=1, R=2, A=3, N=4 }; };

    // Colorspace tag types.
    struct AGG_API linear {};
    struct AGG_API sRGB {};
    ...
    struct AGG_API rgba16
    {
        ...
        {
            self_type ret;
            agg::rgba16::value_type ik = (agg::rgba16::value_type)uround(k * base_mask);
            ret.r = lerp(r, c.r, ik);
            ret.g = lerp(g, c.g, ik);
            ret.b = lerp(b, c.b, ik);
            ret.a = lerp(a, c.a, ik);
        ...
        AGG_INLINE void add(const self_type& c, unsigned cover)
        {
            ...
            {
                cr = r + mult_cover(c.r, (agg::cover_type)cover);
                cg = g + mult_cover(c.g, (agg::cover_type)cover);
                cb = b + mult_cover(c.b, (agg::cover_type)cover);
                ca = a + mult_cover(c.a, (agg::cover_type)cover);
            }
            r = (value_type)((cr > calc_type(base_mask)) ? calc_type(base_mask) : cr);
            g = (value_type)((cg > calc_type(base_mask)) ? calc_type(base_mask) : cg);
            b = (value_type)((cb > calc_type(base_mask)) ? calc_type(base_mask) : cb);
    ...
    struct AGG_API rgba32
    {
        ...
        static AGG_INLINE value_type lerp(value_type p, value_type q, value_type a) 
        {
			...
			// the corArea result at either extreme.
			return (1 - a) * p + a * q;
        }
        ...
        AGG_INLINE void add(const self_type& c, unsigned cover)
        {
            ...
            {
                r += mult_cover(c.r, (agg::cover_type)cover);
                g += mult_cover(c.g, (agg::cover_type)cover);
                b += mult_cover(c.b, (agg::cover_type)cover);
                a += mult_cover(c.a, (agg::cover_type)cover);
            }
            if (a > 1) a = 1;
            if (r > a) r = a;
            if (g > a) g = a;
