...
namespace svg
{
    ...
    struct AGG_API path_attributes
    {
        unsigned     index;
        std::string       fill_scolor;
        std::string       stroke_scolor;
        Color        fill_color;
        Color        stroke_color;
        bool         fill_flag;
        bool         stroke_flag;
        ...
        path_attributes(const path_attributes& attr) :
            ...
            fill_scolor(attr.fill_scolor),
            stroke_scolor(attr.stroke_scolor)
        {
        }
    ...
    public:
        ...
        path_renderer();
        path_renderer(std::map<std::string, Color>* colors);
...
//        }
        ...
        void fill(const Color& f);
        void fill(const char* f);
        void stroke(const Color& s);
        void stroke(const char* s);
                    ...
                    byte opacity=0xff)
        {
            ...
            for(i = 0; i < m_attr_storage.size(); i++)
            {
                ...
                if(attr.fill_flag)
                {
                    ...
                    if (m_colors && m_colors->count(attr.fill_scolor) > 0)
                        color = (*m_colors)[attr.fill_scolor];
                    else color = attr.fill_color;

                    int a = int(color.opacity() * opacity);
                    color.opacity(((a)+1 + (((a)+1) >> 8)) >> 8);

                    ren.color(color.to_rgba8());
                ...
                if(attr.stroke_flag)
                {
                    ...
                    if (m_colors && m_colors->count(attr.stroke_scolor) > 0)
                        color = (*m_colors)[attr.stroke_scolor];
                    else 
