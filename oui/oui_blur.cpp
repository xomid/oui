	...
	if (step == 1)
	{
		for (y = 0; y < h; y++)
		{
			...
			for (i = 0; i <= radius; i++)
			{
				...
				sum_r += (unsigned long)(src_ptr[0] * (i + 1));
				sum_g += (unsigned long)(src_ptr[1] * (i + 1));
				sum_b += (unsigned long)(src_ptr[2] * (i + 1));
				sum_a += (unsigned long)(src_ptr[3] * (i + 1));

				sum_out_r += (unsigned long)(src_ptr[0]);
				sum_out_g += (unsigned long)(src_ptr[1]);
				sum_out_b += (unsigned long)(src_ptr[2]);
				sum_out_a += (unsigned long)(src_ptr[3]);
			}
			...
			for (i = 1; i <= radius; i++)
			{
				...
				stack_ptr[2] = src_ptr[2];
				stack_ptr[3] = src_ptr[3];
				sum_r += (unsigned long)(src_ptr[0] * (radius + 1 - i));
				sum_g += (unsigned long)(src_ptr[1] * (radius + 1 - i));
				sum_b += (unsigned long)(src_ptr[2] * (radius + 1 - i));
				sum_a += (unsigned long)(src_ptr[3] * (radius + 1 - i));
				sum_in_r += (unsigned long)(src_ptr[0]);
				sum_in_g += (unsigned long)(src_ptr[1]);
				sum_in_b += (unsigned long)(src_ptr[2]);
				sum_in_a += (unsigned long)(src_ptr[3]);
			}
			...
			for (x = 0; x < w; x++)
			{
				...
				dst_ptr[3] = byte((sum_a * mul_sum) >> shr_sum);
	...
	else if (step == 2)
	{
		for (x = 0; x < w; x++)
		{
			...
			for (y = 0; y < h; y++)
			{
				...
				sum_out_r -= (unsigned long)(stack_ptr[0]);
				sum_out_g -= (unsigned long)(stack_ptr[1]);
				sum_out_b -= (unsigned long)(stack_ptr[2]);
				sum_out_a -= (unsigned long)(stack_ptr[3]);
				...
				sum_in_r -= (unsigned long)(stack_ptr[0]);
				sum_in_g -= (unsigned long)(stack_ptr[1]);
				sum_in_b -= (unsigned long)(stack_ptr[2]);
				sum_in_a -= (unsigned long)(stack_ptr[3]);
			}
		}
	}
	...
	if (step == 1)
	{
		for (y = 0; y < h; y++)
		{
			...
			for (i = 0; i <= radius; i++)
			{
				stack_ptr = &stack[i];
				stack_ptr[0] = src_ptr[0];
				sum_g += (unsigned long)(src_ptr[0] * (i + 1));
				sum_out_g += (unsigned long)(src_ptr[0]);
			}
			...
			for (i = 1; i <= radius; i++)
			{
				...
				sum_g += (unsigned long)(src_ptr[0] * (radius + 1 - i));
				sum_in_g += (unsigned long)(src_ptr[0]);
			}

			sp = radius;
			xp = radius;
			if (xp > wm) xp = wm;
			src_ptr = src + xp + y * pitch;
			dst_ptr = src + y * pitch;
			for (x = 0; x < w; x++)
			{
				dst_ptr[0] = byte((sum_g * mul_sum) >> shr_sum);
				dst_ptr++;
				...
				if (sp >= div) sp = 0;
				stack_ptr = &stack[sp];
				sum_out_g += (unsigned long)(stack_ptr[0]);
				sum_in_g -= (unsigned long)(stack_ptr[0]);
			}
		}
	}
