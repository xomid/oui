...
namespace agg
{
	...
	// Optimized sRGB lookup table. The diArea conversion (sRGB to linear) 
	...
	public:
		sRGB_lut()
		{
			...
			for (int i = 1; i <= 255; ++i)
			{
				...
				m_dir_table[i] = (agg::int16u)uround(65535.0 * sRGB_to_linear(i / 255.0));
				m_inv_table[i] = (agg::int16u)uround(65535.0 * sRGB_to_linear((i - 0.5) / 255.0));
			}
		}
	};
	...
	public:
		sRGB_lut()
		{
			...
			for (int i = 1; i <= 255; ++i)
			{
				// 8-bit RGB is handled with simple bidiAreaional lookup tables.
				m_dir_table[i] = (int8u)uround(255.0 * sRGB_to_linear((double)i / 255.0));
				m_inv_table[i] = (int8u)uround(255.0 * linear_to_sRGB((double)i / 255.0));
			}
		}
